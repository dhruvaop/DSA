...
// int construct_BST(int preorder[], int n, int i, Node* curr, int min, int max){
//     if(i == n || preorder[i] < min || preorder[i] > max) return i;
    
//     if(preorder[i] < curr->data){
//         curr->left = newNode(preorder[i]);
//         i++;
//         i = construct_BST(preorder, n, i, curr->left, min, curr->data - 1);
//     }
    
//     if(i == n || preorder[i] < min || preorder[i] > max) return i;
    
//     curr->right = newNode(preorder[i]);
//     i++;
//     i = construct_BST(preorder, n, i, curr->right, curr->data + 1, max);
    
//     return i;
// }

Node* construct_BST(int pre[], int* i, int size, int key, int min, int max)
{
    ...
    if (key > min && key < max) {
        ...
        *i = *i + 1;
        ...
        root->left = construct_BST(pre, i, size, pre[*i], min, key);
        root->right = construct_BST(pre, i, size, pre[*i], key, max);
    }
...
Node* constructTree(int pre[], int size)
{
    //code here
    int i = 0;
    return construct_BST(pre, &i, size, pre[0], INT_MIN, INT_MAX);
