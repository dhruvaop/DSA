...
{
    fast;
    string str1 = "geekforgeeks", str2 = "ekegorfkeegsgeek";
    int maxNo = 127, count = 0;
    int arr[maxNo + 1] = {0};

    for(int i = 0; i < str2.length(); i++) arr[int(str2[i])]++;

    for(int i = 0; i < str1.length(); i++) if(arr[int(str1[i])]) count++;

    if(count == str1.length()) yes;
    else no;
